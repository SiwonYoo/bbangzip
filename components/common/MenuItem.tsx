  ...
  return (
    <>
      <Link
        ...
        className={`rounded-xl border border-accentgold bg-offwhite flex gap-4 items-center p-4 ${
          menu.disabled ? "cursor-default opacity-50 pointer-events-none" : ""
        }`}
      >
        <Image src={menu.imgPath} width={70} height={70} alt={`${menu.title} 아이콘`} className="h-15 object-contain" />
        <span className="flex flex-col">
          <span className="text-xl text-t-primary">{menu.title}</span>
